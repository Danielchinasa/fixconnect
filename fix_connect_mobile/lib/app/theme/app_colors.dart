import 'package:flutter/material.dart';

class AppColors {
  // Brand colors
  static const primaryLight = Color(0xFF0dd0f0);
  static const primaryDark = Color(0xFFC8F600);
  static const secondary = Color(0xFF22C55E);
  static Color grey900 = Color(0xff212121);
  static Color grey800 = Color(0xff424242);
  static Color grey700 = Color(0xff616161);
  static Color grey600 = Color(0xff757575);
  static Color grey500 = Color(0xff9e9e9e);
  static Color grey400 = Color(0xffbdbdbd);
  static Color grey300 = Color(0xffe0e0e0);
  static Color grey200 = Color(0xffeeeeee);
  static Color grey100 = Color(0xfff5f5f5);

  // Light mode
  static const lightBackground = Color(0xFFFFFFFF);
  static const lightText = Color(0xFF212121);
  static const surfaceSelectedLight = Color(0xFFF1F3F5);
  static const surfaceLight = Color(0xffEEEEEE);

  // Dark mode
  static const darkBackground = Color(0xff181A20);
  static const darkText = Color(0xFFF9FAFB);
  static const surfaceSelectedDark = Color(0x14C8F600);
  static const surfaceDark = Color(0xff35383F);

  // Common
  static const error = Color(0xFFEF4444);
}
