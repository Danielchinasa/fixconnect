"""Rewrite the three large pages using extracted widgets."""
import os
...
# ── artisan_profile_page.dart ─────────────────────────────────────────────────
write(f'{BASE}/features/home/presentation/pages/artisan_profile_page.dart', """
    ...
    Navigator.of(context).push(
      PageRouteBuilder<void>(
        ...
        pageBuilder: (_, __, ___) =>
            AvatarFullscreenPage(artisan: artisan, badgeColor: artisan.badgeColor),
        ...
        body: CustomScrollView(
          ...
          slivers: [
            SliverAppBar(
              ...
              actions: [
                ProfileCircleIconButton(icon: Icons.share_rounded, onTap: () {},
                    isCollapsed: _isCollapsed, isDark: isDark),
                const SizedBox(width: 6),
                ProfileCircleIconButton(icon: Icons.bookmark_border_rounded, onTap: () {},
                    isCollapsed: _isCollapsed, isDark: isDark),
            ...
            SliverToBoxAdapter(child: _StatsRow(artisan: artisan,
                textColor: textColor, surfaceColor: surfaceColor, primary: primary)),
            SliverToBoxAdapter(child: SizedBox(height: AppSpacing.custom16)),
            SliverToBoxAdapter(child: _InfoRow(artisan: artisan,
                textColor: textColor, surfaceColor: surfaceColor, primary: primary)),
            SliverToBoxAdapter(child: SizedBox(height: AppSpacing.custom16)),
            ...
            SliverToBoxAdapter(child: SizedBox(height: AppSpacing.custom16)),
            SliverToBoxAdapter(child: ArtisanWorkSamplesSection(samples: samples)),
            SliverToBoxAdapter(child: SizedBox(height: AppSpacing.custom16)),
            SliverToBoxAdapter(child: ArtisanReviewsSection(artisan: artisan, reviews: reviews)),
            SliverToBoxAdapter(
...
class _StatsRow extends StatelessWidget {
  ...
  const _StatsRow({
    required this.artisan, required this.textColor,
    required this.surfaceColor, required this.primary,
  });

  @override
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.fromLTRB(AppSpacing.custom16, AppSpacing.custom16, AppSpacing.custom16, 0),
      padding: const EdgeInsets.symmetric(vertical: 20),
      decoration: BoxDecoration(color: surfaceColor, borderRadius: BorderRadius.circular(20)),
      child: IntrinsicHeight(
        child: Row(children: [
          Expanded(child: ProfileStatCell(icon: Icons.star_rounded, iconColor: const Color(0xFFFFB800),
              value: artisan.rating.toStringAsFixed(1), label: 'Rating', textColor: textColor)),
          VerticalDivider(width: 1, thickness: 1, color: textColor.withOpacity(0.12)),
          Expanded(child: ProfileStatCell(icon: Icons.handyman_rounded, iconColor: primary,
              value: artisan.completedJobs > 0 ? '${artisan.completedJobs}' : '–',
              label: 'Jobs Done', textColor: textColor)),
          VerticalDivider(width: 1, thickness: 1, color: textColor.withOpacity(0.12)),
          Expanded(child: ProfileStatCell(icon: Icons.chat_bubble_rounded,
              iconColor: AppColors.secondary, value: '${artisan.reviews}',
              label: 'Reviews', textColor: textColor)),
        ]),
      ),
    );
  }
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: ProfileInfoCard(icon: Icons.location_on_rounded, iconColor: primary,
            label: 'Location',
            value: artisan.location.isNotEmpty ? artisan.location : 'Lagos, Nigeria',
            surfaceColor: surfaceColor, textColor: textColor)),
        SizedBox(width: AppSpacing.custom8),
        Expanded(child: ProfileInfoCard(icon: Icons.payments_rounded, iconColor: AppColors.secondary,
            label: 'Starting Price', value: artisan.startingPrice,
            surfaceColor: surfaceColor, textColor: textColor)),
      ]),
    );
  }
...
write(f'{BASE}/features/home/presentation/pages/service_detail_page.dart', """
import 'package:fix_connect_mobile/app/theme/app_colors.dart';
import 'package:fix_connect_mobile/core/utils/build_context_ext.dart';
import 'package:fix_connect_mobile/features/home/data/datasources/home_mock_datasource.dart';
        ...
        children: [
          CustomScrollView(
            ...
            slivers: [
              ...
              SliverToBoxAdapter(
                child: ServiceStatsStrip(service: service),
              ),
              SliverToBoxAdapter(
